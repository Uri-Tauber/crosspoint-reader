...
void KOReaderSyncActivity::performSync() {
  ...
  xSemaphoreTake(renderingMutex, portMAX_DELAY);
  statusMessage = TR(FETCH_PROGRESS);
  xSemaphoreGive(renderingMutex);
  updateRequired = true;
...
void KOReaderSyncActivity::performUpload() {
  xSemaphoreTake(renderingMutex, portMAX_DELAY);
  state = UPLOADING;
  statusMessage = TR(UPLOAD_PROGRESS);
  xSemaphoreGive(renderingMutex);
  updateRequired = true;
  vTaskDelay(10 / portTICK_PERIOD_MS);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    xTaskCreate(
        [](void* param) {
          ...
          self->statusMessage = TR(CALC_HASH);
...
void KOReaderSyncActivity::render() {
  ...
  if (state == NO_CREDENTIALS) {
    renderer.drawCenteredText(UI_10_FONT_ID, 280, TR(NO_CREDENTIALS_MSG), true, EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, 320, TR(KOREADER_SETUP_HINT));
  ...
  if (state == SHOWING_RESULT) {
    ...
    const std::string remoteChapter = (remoteTocIndex >= 0)
                                          ...
                                          : (std::string(TR(SECTION_PREFIX)) + std::to_string(remotePosition.spineIndex + 1));
    const std::string localChapter = (localTocIndex >= 0) ? epub->getTocItem(localTocIndex).title
                                                          : (std::string(TR(SECTION_PREFIX)) + std::to_string(currentSpineIndex + 1));

    // Remote progress - chapter and page
    renderer.drawText(UI_10_FONT_ID, 20, 160, TR(REMOTE_LABEL), true);
    char remoteChapterStr[128];
    snprintf(remoteChapterStr, sizeof(remoteChapterStr), "  %s", remoteChapter.c_str());
    renderer.drawText(UI_10_FONT_ID, 20, 185, remoteChapterStr);
    char remotePageStr[64];
    snprintf(remotePageStr, sizeof(remotePageStr), TR(PAGE_OVERALL_FORMAT), remotePosition.pageNumber + 1,
             remoteProgress.percentage * 100);
    renderer.drawText(UI_10_FONT_ID, 20, 210, remotePageStr);

    if (!remoteProgress.device.empty()) {
      char deviceStr[64];
      snprintf(deviceStr, sizeof(deviceStr), TR(DEVICE_FROM_FORMAT), remoteProgress.device.c_str());
      renderer.drawText(UI_10_FONT_ID, 20, 235, deviceStr);
    }

    // Local progress - chapter and page
    renderer.drawText(UI_10_FONT_ID, 20, 270, TR(LOCAL_LABEL), true);
    char localChapterStr[128];
    snprintf(localChapterStr, sizeof(localChapterStr), "  %s", localChapter.c_str());
    renderer.drawText(UI_10_FONT_ID, 20, 295, localChapterStr);
    char localPageStr[64];
    snprintf(localPageStr, sizeof(localPageStr), TR(PAGE_TOTAL_OVERALL_FORMAT), currentPage + 1, totalPagesInSpine,
    ...
    renderer.drawText(UI_10_FONT_ID, 20, optionY, TR(APPLY_REMOTE), selectedOption != 0);

    // Upload option
    if (selectedOption == 1) {
      renderer.fillRect(0, optionY + optionHeight - 2, pageWidth - 1, optionHeight);
    }
    renderer.drawText(UI_10_FONT_ID, 20, optionY + optionHeight, TR(UPLOAD_LOCAL), selectedOption != 1);
  ...
  if (state == NO_REMOTE_PROGRESS) {
    renderer.drawCenteredText(UI_10_FONT_ID, 280, TR(NO_REMOTE_MSG), true, EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, 320, TR(UPLOAD_PROMPT));
  ...
  if (state == UPLOAD_COMPLETE) {
    renderer.drawCenteredText(UI_10_FONT_ID, 300, TR(UPLOAD_SUCCESS), true, EpdFontFamily::BOLD);
  ...
  if (state == SYNC_FAILED) {
    renderer.drawCenteredText(UI_10_FONT_ID, 280, TR(SYNC_FAILED_MSG), true, EpdFontFamily::BOLD);
