...
void ButtonRemapActivity::render() {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, TR(REMAP_FRONT_BUTTONS), true, EpdFontFamily::BOLD);
  renderer.drawCenteredText(UI_10_FONT_ID, 40, TR(REMAP_PROMPT));
  ...
  for (uint8_t i = 0; i < kRoleCount; i++) {
    ...
    const char* assigned = (tempMapping[i] == kUnassigned) ? TR(UNASSIGNED) : getHardwareName(tempMapping[i]);
  ...
  renderer.drawCenteredText(SMALL_FONT_ID, 250, TR(REMAP_RESET_HINT), true);
  renderer.drawCenteredText(SMALL_FONT_ID, 280, TR(REMAP_CANCEL_HINT), true);
...
bool ButtonRemapActivity::validateUnassigned(const uint8_t pressedButton) {
  ...
  for (uint8_t i = 0; i < kRoleCount; i++) {
    if (tempMapping[i] == pressedButton && i != currentStep) {
      errorMessage = TR(ALREADY_ASSIGNED);
