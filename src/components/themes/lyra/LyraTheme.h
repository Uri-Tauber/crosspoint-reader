...
//   void drawProgressBar(const GfxRenderer& renderer, Rect rect, size_t current, size_t total) override;
  ...
  void drawRecentBookCover(GfxRenderer& renderer, Rect rect, std::vector<RecentBookInfo> recentBooks, const int selectorIndex,
    bool& coverRendered, bool& coverBufferStored, bool& bufferRestored, std::function<bool()> storeCoverBuffer) override;
};
